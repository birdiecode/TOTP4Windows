...
if ($Service) {                 # Run the service
  ...
  try {
    ...
    do { # Keep running until told to exit by the -Stop handler
      ...
      switch ($message) {
        ...
        "TimerTick" { # Example. Periodic event generated for this example
          $passValTOTP = Get-TimeBasedOneTimePassword -SharedSecret $secretTOTP
          $passValTOTPSecure = ConvertTo-SecureString $passValTOTP -AsPlainText -Force
          Set-LocalUser -Name $userTOTP -Password $passValTOTPSecure
        }
    ...
    $passValSecure = ConvertTo-SecureString $passwordNoTOTP -AsPlainText -Force
    Set-LocalUser -Name $userTOTP -Password $passValSecure
